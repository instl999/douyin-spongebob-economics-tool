...
| **整片速度** | 「1.5×（默认，短视频节奏）」/「1.0×（自然语速）」/「1.25×」/「1.75×」 | **1.5×** |

补充说明：

- **时长不是随便定的。** 文案多长，视频就多长。选了目标时长，整片速度会在 0.5×–2.0× 之间自动拟合；超出这个范围就拟合不了，Step 2 会直接告诉你差多少字。**这时要回来问用户：是改文案，还是接受实际时长。不要自己删改文案。**
- **整片速度默认 1.5×，管的是整条片子**，不是只有语速：配音、每镜时长、卡片、溶解、字幕一起变快，所以不会出现「声音跑完了画面还杵着」。1.0× 是基准。用户说了目标时长的话，目标时长优先，速度由它拟合出来，这一项就不用再问。
...
- **画风选「新建一套」时**：复制 `casts/_template.json`（里面的 `_hint_*` 写清了每个字段该怎么填、坑在哪）成 `casts/<key>.json`，填好后先 `python scripts/build.py --check` 验一遍，再 `python scripts/build_library.py <key> --plates` 把每个角色的定妆参考图画出来。想要中文名就在 `casts/styles.json` 的 `styles` 里加一条 `{"<key>": {"label": "中文名", "note": "一句话"}}`。**要提前告诉用户这一步的量**：一个角色一张、每张约 20 秒，三五个角色也就一两分钟。参考图只画这一次；片子里其余的画面都是按台词现画的，不复用。
...
| 成片时长 | project 的 `target_seconds`；整片速度在 0.5×–2.0× 内自动拟合，超出范围会报出来 |
| **整片速度** | project 的 `speed`，**默认 1.5**，也可以 `--speed 1.2`。1.0 是基准。配音、镜头长度、卡片、溶解、字幕一起变快，不是只有配音变快 |
| **开场标题念不念** | **默认念，而且没有开关。** 标题卡以前只画不念，每条片子开头都是两秒半的无声大字。现在标题和别的句子一样配音，卡片停留时间跟着这句话的长度走 |
